...
class GuardianAPIService {
  ...
  async fetchArticles(options = {}) {
    ...
    try {
      ...
      console.log("Fetching The Guardian articles with options");
      ...
      let pageIndex = 0;
      for (let page = fromPage; page < pages + fromPage; page++) {
        const params = {
          "api-key": this.apiKey,
          "page-size": pageSize,
          page: page,
        ...
        console.log(
          `Fetching articles from The Guardian API (${++pageIndex}/${pages})...`
        );
        const response = await axios.get(`${this.baseUrl}/search`, { params });

        if (response.data.response.status === "ok") {
          const articles = this.normalizeArticles(
            response.data.response.results
          );
          allArticles = allArticles.concat(articles);
          console.log(
            `✓ Fetched ${articles.length} articles from page ${page}`
          );
        } else {
          throw new Error(
            `Guardian API error: ${response.data.response.message}`
          );
        }
    ...
    return articles
      .filter((article) => {
        ...
        const hasContributor =
          article.tags &&
          article.tags.some((tag) => tag.type === "contributor");
      ...
      .map((article) => {
        ...
        const author = contributor
          ? this.extractAuthorName(contributor)
          : "Unknown";
        ...
        return {
          ...
          contributorBio: contributor?.bio
            ? this.stripHtml(contributor.bio)
            : null,
  ...
  getMockArticles() {
    return [
      {
        title:
          "Battery Startup Revolutionizes EV Supply Chain with Silicon Innovation",
        description:
          "A new battery technology startup is using domestically-sourced metallurgical silicon to create breakthrough materials for electric vehicles.",
        content:
          "A pioneering battery technology company has announced a major breakthrough in electric vehicle battery production using domestically-sourced metallurgical silicon. The innovation promises to reduce costs and strengthen the US supply chain for critical EV components.",
      ...
      {
        title:
          "Restaurant Robotics Sees $12M Investment as Labor Crisis Continues",
        description:
          "Automation technology for quick-serve restaurants raises significant seed funding as industry faces ongoing labor challenges.",
        content:
          "The restaurant industry is turning to robotics and automation as a solution to persistent labor shortages. One startup has raised $12M in seed funding to deploy its platform across quick-serve operations nationwide.",
      ...
      {
        title:
          "Fintech Platform Partners with AWS to Transform Mortgage Processing",
        description:
          "Major cloud partnership promises to reduce costs and improve compliance in mortgage lending technology.",
        content:
          "A leading mortgage technology platform has announced a strategic partnership with Amazon Web Services to modernize its infrastructure. The move is expected to significantly reduce latency and costs while improving compliance capabilities.",
      ...
      },
      {
        ...
        description:
          "Domestic supply chain initiatives drive new wave of climate technology investments.",
        content:
          "Investment in climate technology companies focused on US manufacturing has reached new highs. The trend is driven by renewed emphasis on domestic supply chains and materials innovation.",
      ...
      {
        title:
          "Robotics Automation Tackles Quick-Service Restaurant Operations",
        description:
          "New technology platform brings industrial automation to fast food industry.",
        content:
          "The fast food industry is embracing robotics automation to streamline operations and address labor challenges. New platforms are making industrial automation accessible to quick-service restaurants.",
